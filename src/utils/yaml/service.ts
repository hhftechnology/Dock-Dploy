// Build the Docker Compose `services.<name>` block for one service.
// Pure mapping of ServiceConfig → plain JS object that yamlStringify can emit.
...
const NETWORK_MODE_VPN_TYPES = ["tailscale", "cloudflared"];

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type ComposeServiceEntry = Record<string, any>;

export function buildServiceEntry(
  svc: ServiceConfig,
  vpn: VPNConfig,
  vpnServiceName: string | null,
): ComposeServiceEntry {
  const shouldUseVpn =
    vpn.enabled && vpn.type !== null && vpn.servicesUsingVpn.includes(svc.name);

  const usesNetworkMode =
    vpn.enabled &&
    vpn.type !== null &&
    NETWORK_MODE_VPN_TYPES.includes(vpn.type) &&
    shouldUseVpn;
  ...
  const entry: ComposeServiceEntry = {
      ...
      : svc.ports.length
        ? svc.ports
            .map((p) => {
              ...
              const portStr =
                p.host && p.container ? `${p.host}:${p.container}` : p.container;
        ...
        : undefined,
    expose:
      svc.expose && svc.expose.length > 0 ? svc.expose.filter(Boolean) : undefined,
    network_mode:
        ...
        ? svc.volumes
            .map((v) => {
              if (v.host && v.container) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const vol: any = { type: "bind", source: v.host, target: v.container };
                if (v.read_only) vol.read_only = true;
                return vol;
              } else if (v.container) {
                return { type: "volume", target: v.container };
              }
              return undefined;
            })
            .filter(Boolean)
    ...
    extra_hosts:
      svc.extra_hosts && svc.extra_hosts.filter(Boolean).length
        ...
        : undefined,
    dns:
      svc.dns && svc.dns.filter(Boolean).length ? svc.dns.filter(Boolean) : undefined,
      ...
      svc.ulimits && svc.ulimits.filter((u) => u.name).length
        ? svc.ulimits
            ...
            .reduce(
              (acc, u) => {
                if (u.soft && u.hard) {
                  acc[u.name] = { soft: parseInt(u.soft), hard: parseInt(u.hard) };
                } else if (u.soft) {
    ...
    deploy: buildDeployBlock(svc),
  };

  // Strip undefined keys so yamlStringify doesn't emit them.
  Object.keys(entry).forEach((k) => entry[k] === undefined && delete entry[k]);
  return entry;
}

function buildDeployBlock(svc: ServiceConfig) {
  if (!svc.deploy || !svc.deploy.resources) return undefined;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const limits: any = {};
  if (svc.deploy.resources.limits?.cpus) limits.cpus = svc.deploy.resources.limits.cpus;
  ...
  if (svc.deploy.resources.reservations?.memory)
    reservations.memory = svc.deploy.resources.reservations.memory;
